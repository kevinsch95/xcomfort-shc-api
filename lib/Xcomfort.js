 ...
 */
  ...
  * @param {string} params.remoteKey - remote access key to SHC
  ...
  * @prop {string} remoteKey - remote access key from params
...
class Xcomfort extends EventEmitter {
  constructor({
    ...
    if(!remoteKey) throw new Error('No remote access key supplied');
  ...
  login() {
    ...
    return new Promise( (resolve, reject) => {
      request({
        url: this.baseUrl + '/remote_access/login',
        method: 'post',
        form: {
          rakey: this.remoteKey,
          remotable_user: this.username,
  ...
  dimDarker(deviceName, cb) {
    return checkIfDeviceExists.call(this, deviceName, deviceTypes.DIM_ACTUATOR)
      .then( ({id, type}) => invokeDCOOperation.call(this, id, type, 'darker'))
      .asCallback(cb);
  }
  /**
  ...
  dimStop(deviceName, cb) {
    return checkIfDeviceExists.call(this, deviceName, deviceTypes.DIM_ACTUATOR)
      .then( ({id, type}) => invokeDCOOperation.call(this, id, type, 'stop'))
      .asCallback(cb);
  }
  /**
   * Triggers scene
